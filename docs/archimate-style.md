...
This is rectangle + decorator-icon notation, not full Archi figure skins, not custom themes, and not Open Exchange styling. The `.plein` `styles { }` block is still ignored. `plein import` drops Open Exchange fill and line colours, and `plein export-open-exchange` does not write them; see [Open Exchange import and export](open-exchange-import.md).
