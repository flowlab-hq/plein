...
1. Golden (exit 0): `valid-basic.plein`, `valid-views.plein`, `valid-catalogue-layers.plein`, `valid-capability-value-stream.plein`, `valid-value-stream-stages.plein`, `samples/value-stream-demo.plein`
...
The Mac app has a longer Open → view, switch, and reload checklist in [app/README.md](../../app/README.md). Load→list is covered by `npm test` (`src/list-model.test.ts`). Left-sidebar layout (no bottom strip) is `src/app-layout.test.ts`. Viewpoint layout membership (golden include/exclude) is `src/layout.test.ts` / `./scripts/assert-viewpoint-layout.sh`. ArchiMate type colours and icons are `src/archimate-style.test.ts` / `./scripts/assert-archimate-style.sh`. Edit → reload → diagram is `src/reload.test.ts` / `./scripts/assert-reload-diagram.sh`. One model → many views is `src/browser.test.ts` / `./scripts/assert-multi-view-browser.sh`.
