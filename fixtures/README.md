...
- `samples/research-data-eprints-arkivum.plein` — Mac / README demo for **Arran S4 named-view jump** (Plein v0.1.5+): Sussex research-data / ePrints / Arkivum style. Business actor + two processes, application components (project, paper, data, ePrints), and technology (data artifact, file storage, archive, storage platform, Arkivum). Two viewpoints — **Public access / discovery** and **Research project and storage** — so the View chrome above the canvas names the current view. Expected to pass with exit code 0.
